...
public class ConeAligner : MonoBehaviour
{
    ...
    public void AlignCone(Vector3 startPoint, Vector3 endPoint, int coneNumber)
    {
        ...
        cone.tag = "TransparentTarget";
        // 円錐のスケールを調整（距離に応じてY軸方向に拡大）
        Vector3 currentScale = cone.transform.localScale;
        cone.transform.localScale = new Vector3(currentScale.x, currentScale.y, distance * 100f * 50f);
        int r=0;
        int g=0;
        int b=0;
        if(0 <= coneNumber)
    ...
    // {
    //     if (!showDebugVisuals || !hasValidPoints) return;
    ...
    // 外部から強制的に再配置するためのパブリックメソッド
//     public void ForceAlign()
//     {
//         if (fileOperation != null && fileOperation.modelPositions != null && fileOperation.modelPositions.Length >= 2)
//         {
//             if(trajectoryIndex >= 0 && trajectoryIndex < 710)
//             {
//                 startPoint = fileOperation.modelPositions[trajectoryIndex];
//                 endPoint = fileOperation.modelPositions[trajectoryIndex + 10];
//                 hasValidPoints = true;
//                 AlignCone();
//             }
//             else
//             {
//                 Debug.Log("trajectoryIndex is out of range.");
//             }
